...
TileLayer * LayerParser::parseTileLayer(const char * inputFile)
{
	...
	TileLayer* pTileLayer = new TileLayer(tile.tileWidth, tile.tileHeight, m_tilesets);
	...
	for (TiXmlElement* e = pRoot->FirstChildElement(); e != NULL; e = e->NextSiblingElement())
	{
		if (e->Value() == std::string("layer"))
		{
				...
				NULL; c = c->NextSiblingElement())
			{
				...
				{
					for (TiXmlNode* pDataNode = c->FirstChild(); pDataNode != NULL; pDataNode =
						pDataNode->NextSibling())
					{
