...
void MenuButton::handleEvents(SDL_Event e) {
	...
	if (mousePos.getX() >= this->position.getX() - (this->width / 2)
		...
		&& mousePos.getY() <= this->position.getY() + (this->height / 2)) {
		//HOVER
		if (spriteRow != MOUSE_PRESSED) spriteRow = MOUSE_OVER;
		//PRESSED
		if (input->getMouseButtonStateDown(e, SDL_BUTTON_LEFT))
			spriteRow = MOUSE_PRESSED;
		else if (input->getMouseButtonStateUp(e, SDL_BUTTON_LEFT)) {
