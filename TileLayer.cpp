...
void TileLayer::render() {
	...
	for (int i = 0; i < m_numRows; i++)
	{
		for (int j = 0; j < m_numColumns; j++)
		{
			...
			id--;
			TheTextureManager->drawTile(tileset.name, 0, 0,	(j * m_tileSize) - x2, (i * m_tileSize) - y2, m_tileSize, m_tileSize, (id - (tileset.firstGridID - 1)) /
				tileset.numColumns, (id - (tileset.firstGridID - 1)) % tileset.numColumns, TheGame->getRenderer());
		}
	}
};
